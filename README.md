...
- `agenda.html` — the weekly client catch-up agenda (`/agenda`). **Deliberately hidden:
  ...
  during the meeting.
- `sku-checklist.html` — the per-SKU review checklist, in the tab bar and also linked
  from the final section of `catalogue-audit.html`. Client-safe: walks one product
  through SKU check, the three visibility gates, tags → collections, filter metafields,
  variants & voltage, imagery and a storefront pass. Conventions in it (tag slugs,
  metafield names, gate list) mirror the CMS manual — if those change in `index.html`,
  update this page too. Deliberately does NOT mention the internal hold tags
  (`arbt-*`/`scope-review`) — David, 2026-08-11: too confusing for the client here.
  There is no separate events guide page: the events how-to is CMS Manual §3.4, now
  surfaced as its own entry in the manual's table of contents (`#events-calendar`).
